function doGet(e) {
  ...
  const handlers = { health: function() { return jsonOk_({ service: 'pwa-avaliacao-idosos' }); }, listPeople: listPeople, getPerson: getPerson, getPersonFlow: getPersonFlow, getAssessment: getAssessment, getHistory: getHistory, getHistorySummary: getHistorySummary, getCatalog: getCatalog };
...
function doPost(e) {
  try {
    ...
    const handlers = { savePerson: savePerson, createAssessment: createAssessment, saveAssessment: saveAssessment, completeAssessment: completeAssessment, archiveAssessment: archiveAssessment, generateReport: generateReport };
