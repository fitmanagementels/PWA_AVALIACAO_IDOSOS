...
export async function request(action, payload = {}, method = 'POST') {
  ...
  try {
    const response = await fetch(url, method === 'GET' ? { method, signal: controller.signal } : { method, body: JSON.stringify({ action, payload }), headers: { 'Content-Type': 'text/plain;charset=utf-8' }, signal: controller.signal });
