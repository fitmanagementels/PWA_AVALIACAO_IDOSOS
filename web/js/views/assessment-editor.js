...
function card(id, assessment, person) {
  ...
  return `<details class="test-card" data-test-id="${id}" data-state="${presentation.state}"><summary><span>${escapeHtml(detail.title || testName(id))}</span><i class="state-marker" aria-label="${presentation.label}"></i></summary><p>${detail.hint}</p>${notCompletedToggle(`${id}-not-completed`, inputs)}<label>Motivo, se não concluído<input name="${id}-reason" placeholder="Dor, insegurança, intercorrência ou recusa"${valueAttribute(inputs, `${id}-reason`)}></label>${fields}${result ? `<p class="result-caption">${presentation.officialText || presentation.label} · ${presentation.label}</p>` : ''}</details>`;
}

function notCompletedToggle(name, inputs) {
  return `<label class="selection-toggle"><input type="checkbox" name="${name}"${checkedAttribute(inputs, name)}><span class="toggle-track" aria-hidden="true"><span></span></span><span>Não concluído</span></label>`;
}
