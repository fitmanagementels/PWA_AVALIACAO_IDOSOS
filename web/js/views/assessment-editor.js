import { addSelectedTests, assessmentReadiness, buildResult, markNotCompleted, removeSelectedTest } from '../assessment-domain.js';
import { TESTS } from '../domain.js';
import { presentationForResult, sessionColorCounts } from '../result-presentation.js';
import { enqueueAssessmentMutation, queueMutation, saveDraft } from '../storage.js';
...
export function renderAssessmentEditor(root, assessment, onBack, onComplete = onBack) {
  ...
  const openTest = (id, origin) => {
    ...
    openTestSheet({
      ...
      onClose: () => {
        ...
        renderAssessmentEditor(root, assessment, onBack, onComplete);
  ...
  root.querySelectorAll('[data-remove-test]').forEach((button) => button.addEventListener('click', async () => {
    const testId = button.dataset.removeTest;
    if (assessment.testIds.length <= 1) {
      alert('Uma avaliação precisa manter ao menos um teste. Para manter este registro, marque o teste como não concluído.');
      return;
    }
    if (!window.confirm('Retirar este teste da avaliação? Os dados e tentativas deste teste serão apagados.')) return;
    assessment.testIds = removeSelectedTest(assessment.testIds, testId);
    assessment.results = (assessment.results || []).filter((result) => result.testId !== testId);
    assessment.draftInputs = replaceTestDraftInputs(assessment.draftInputs, testId, {});
    assessment.hydratedTestIds = (assessment.hydratedTestIds || []).filter((id) => id !== testId);
    await persistAssessment();
    await queueMutation('removeAssessmentTest', { avaliacaoId: assessment.id, testeId: testId });
    window.scheduleSync?.();
    renderAssessmentEditor(root, assessment, onBack, onComplete);
  }));
  ...
  form.onsubmit = async (event) => {
    ...
    try {
      ...
      window.scheduleSync?.();
      message.textContent = 'Rascunho salvo neste aparelho; envio em segundo plano.';
...
function testSummaryCard(id, assessment, person) {
  ...
  const summary = testCardSummary({ testId: id, draftInputs: assessment.draftInputs, result });
  const definition = testDefinition(id);
  return `<article class="test-summary-card" data-state="${escapeHtml(presentation.state)}"><button class="test-summary-card__open" type="button" data-open-test="${escapeHtml(id)}"><span class="test-summary-card__content"><strong>${escapeHtml(definition.title)}</strong><small>${escapeHtml(summary.text)}</small></span><span class="test-summary-card__meta"><i class="state-marker" aria-label="${escapeHtml(presentation.label)}"></i><small>${summary.entered} de ${summary.total}</small><b>Abrir ›</b></span></button>${assessment.status === 'concluida' ? '' : `<button class="test-summary-card__remove" type="button" data-remove-test="${escapeHtml(id)}" aria-label="Retirar ${escapeHtml(definition.title)} da avaliação">Retirar</button>`}</article>`;
}
