...
export function renderAssessmentEditor(root, assessment, onBack) {
  ...
  root.querySelector('form').onsubmit = (event) => { event.preventDefault(); try { const data = new FormData(event.currentTarget); assessment.results = assessment.testIds.map((id) => collectResult(id, data)); assessment.testNotes = data.get('testNotes'); assessment.studentObservations = data.get('studentObservations'); assessment.updatedAt = new Date().toISOString(); localStorage.setItem(`assessment:${assessment.id}`, JSON.stringify(assessment)); root.querySelector('.form-message').textContent = 'Rascunho salvo neste aparelho. Sincronize antes de concluir.'; } catch (error) { root.querySelector('.form-message').textContent = error.message; } };
}
function card(id) {
...
function collectResult(id, data) {
  if (id === 'sppb') return buildResult({ testId: id, unit: 'score', attempts: [{ side: 'caminhada4m', value: Math.min(...[data.get('sppb-gait-1'), data.get('sppb-gait-2')].map(Number).filter(Number.isFinite)) }, { side: 'sentarLevantar5x', value: Number(data.get('sppb-chair')) }, { side: 'equilibrio', value: Number(data.get('sppb-tandem')) }] });
  ...
  const attempts = (detail.sides ? ['direito', 'esquerdo'] : ['unico']).flatMap((side) => Array.from({ length: detail.attempts }, (_, index) => ({ side, value: Number(data.get(`${id}-${side}-${index + 1}`)) })).filter((attempt) => Number.isFinite(attempt.value)));
