...
function renderPersonForm(root) {
  ...
  root.querySelector('form').onsubmit = (event) => { event.preventDefault(); const data = new FormData(event.currentTarget); const person = { id: crypto.randomUUID(), name: data.get('name').trim(), birthDate: data.get('birthDate'), sex: data.get('sex'), whatsapp: String(data.get('whatsapp')).replace(/\D/g, '') }; write([...read(), person]); renderPerson(root, person.id); };
}
...
function renderStart(root, person) {
  ...
  root.querySelector('[data-back]').onclick = () => renderPerson(root, person.id); root.querySelector('form').onsubmit = (event) => { event.preventDefault(); const data = new FormData(event.currentTarget); try { const start = buildAssessmentStart({ personId: person.id, professionalName: data.get('professionalName'), testIds: data.getAll('testIds') }); const assessment = { id: crypto.randomUUID(), personId: person.id, personName: person.name, ...start, date: data.get('date'), status: 'rascunho', results: [] }; localStorage.setItem(`assessment:${assessment.id}`, JSON.stringify(assessment)); renderAssessmentEditor(root, assessment, () => renderPerson(root, person.id)); } catch (error) { root.querySelector('.form-message').textContent = error.message; } };
}
